...
const Model = ({ position, rotation, scale }) => {
  const { scene } = useGLTF("/Images/fisica.glb"); // Ruta a tu modelo GLB
  return (
    <primitive
      object={scene}
      position={position}
...
const ZapparARScenePhysic = () => {
  const [position, setPosition] = useState([0, 0, -0.1]);
  const [rotation, setRotation] = useState([0, -160, 0]);
...
export default ZapparARScenePhysic;
